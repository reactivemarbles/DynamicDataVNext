...
public static partial class CreateForUpdateTests
{
    public static partial class WithMultipleChanges
    {
        public abstract class Base
        {
            ...
            {
                var result = InvokeUut(changes: Array.Empty<OrderedChange<int>>());
            ...
            public void Otherwise_ResultContainsChanges(int changeCount)
            {
                var changes = Enumerable
                    .Range(0, changeCount)
                    .Select(static index => OrderedChange.CreateInsertion(
                        index:  index,
                        item:   index + 1))
                    .ToArray();
            ...
            protected abstract OrderedChangeSet<int> InvokeUut(IEnumerable<OrderedChange<int>> changes); 
        }
    }
}
