    ...
    where TUut : IList<string?>
{
        ...
        int arrayIndex)
    {
        ...
        array[..arrayIndex].Should().BeEquivalentTo(Enumerable.Repeat(default(string), arrayIndex), "array elements outside of the copy bounds should not have been changed");
        array[(arrayIndex + items.Length)..].Should().BeEquivalentTo(Enumerable.Repeat(default(string), array.Length - items.Length - arrayIndex), "array elements outside of the copy bounds should not have been changed");
    }
}
