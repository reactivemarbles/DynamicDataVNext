    ...
    where TUut : ICache<string, TestItem>
{
        ...
        int arrayIndex)
    {
        using var fixture = TUutFixture.Create(
            ...
            items:          Enumerable.Range(1, itemCount)
                .Select(key => new TestItem()
                {
                    Key = key.ToString()
                }));

        var array = new TestItem[arrayLength];
        ...
        int arrayIndex)
    {
        ...
        array[..arrayIndex].Should().BeEquivalentTo(Enumerable.Repeat<TestItem>(null!, arrayIndex), "array elements outside of the copy bounds should not have been changed");
        array[(arrayIndex + items.Length)..].Should().BeEquivalentTo(Enumerable.Repeat<TestItem>(null!, array.Length - items.Length - arrayIndex), "array elements outside of the copy bounds should not have been changed");
    }
}
