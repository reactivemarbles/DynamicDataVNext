...
public static partial class CreateForResetTests
{
    ...
    {
        [TestFixture]
        public sealed class FromEnumerablesOfKeyedItems
            : Base
        {
            [Test]
                ...
                var result = FluentActions.Invoking(() =>
                    {
                        _ = KeyedChangeSet.CreateForReset(
                            removals:   Array.Empty<KeyedItem<int, int>>(),
                            additions:  (null as IEnumerable<KeyedItem<int, int>>)!);
                    })
            ...
            public void WhenRemovalsIsNull_ThrowsException()
            {
                var result = FluentActions.Invoking(() =>
                    {
                        _ = KeyedChangeSet.CreateForReset(
                            removals:   (null as IEnumerable<KeyedItem<int, int>>)!,
                            additions:  Array.Empty<KeyedItem<int, int>>());
                    })
                    .Should().Throw<ArgumentNullException>()
            ...
            protected override KeyedChangeSet<int, int> InvokeUut(
                ...
                => KeyedChangeSet.CreateForReset(
                    removals:   removedItems.Select(SelectKeyedItem),
                    additions:  addedItems.Select(SelectKeyedItem));
        }
    }
}
