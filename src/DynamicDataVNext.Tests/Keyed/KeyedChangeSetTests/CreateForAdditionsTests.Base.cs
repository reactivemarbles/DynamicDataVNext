...
public static partial class CreateForAdditionsTests
{
        ...
        : KeyedChangeSetTests.Base
    {
        ...
        public void Otherwise_ResultIsClear(int itemCount)
        {
            ...
            result.Changes.Select(static change => change.Type).Should().AllBeEquivalentTo(KeyedChangeType.Addition, "an addition change should have been generated for each added item");
            result.Changes.Select(static change => change.AsAddition().Key).Should().BeEquivalentTo(items.Select(SelectKey), static options => options.WithStrictOrdering(), "the given item keys should have been embedded into the generated changes");
            result.Changes.Select(static change => change.AsAddition().Item).Should().BeEquivalentTo(items, static options => options.WithStrictOrdering(), "the given items should have been embedded into the generated changes");
        }
