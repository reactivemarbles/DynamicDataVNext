...
public static partial class CreateForUpdateTests
{
    public static partial class WithMultipleChanges
    {
        public abstract class Base
        {
            ...
            {
                var result = InvokeUut(changes: Array.Empty<DistinctChange<int>>());
            ...
            public void Otherwise_ResultContainsChanges(int changeCount)
            {
                var changes = Enumerable
                    .Range(1, changeCount)
                    .Select(static item => new DistinctChange<int>()
                    {
                        Item = item,
            ...
            protected abstract DistinctChangeSet<int> InvokeUut(IEnumerable<DistinctChange<int>> changes); 
        }
    }
}
